...
class ExecoClient :
    ...
    # ***************************
    # Upload the monitor configuration
    # @params:
    #   - cpuConfig: the configuration of the cpu market (dict)
    #   - memConfig: the configuration of the memory market (dict)
    # ***************************
    def uploadMonitorConfiguration (self, cpuConfig, memConfig):
        print (cpuConfig)
        with open ("/tmp/mem-market.json", 'w') as fp :
            fp.write (json.dumps (memConfig))

        with open ("/tmp/cpu-market.json", 'w') as fp :
            fp.write (json.dumps (cpuConfig))
            
        self.uploadFiles (self._hnodes, ["/tmp/mem-market.json", "/tmp/cpu-market.json"], "/usr/lib/dio/")
    ...
    # ***************************
    # Reset every counters of the running monitors
    # ***************************
    def resetMonitorCounters (self):
        self.launchAndWaitCmd (self._hnodes, "dio-client --reset-counters")
        logger.info ("Monitor counters have been reset")
